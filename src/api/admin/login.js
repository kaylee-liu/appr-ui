...
export function loginByUsername (tenantCode, identifier, credential, code, randomStr) {
  ...
  return request({
    ...
    method: 'post',
    params: { username: identifier, credential: credential, randomStr, code, grant_type: grantType, scope }
  })
}
export function loginByHinanToken (tenantCode, identifier, credential, code, randomStr) {
  const grantType = 'hinan'
  const scope = 'read'
  return request({
    url: '/api/auth/hinan/token',
    headers: {
      'Authorization': basicAuthorization,
      'Tenant-Code': tenantCode
    },
    method: 'post',
    params: { hinanToken: identifier, credential: credential, randomStr, code, grant_type: grantType, scope }
  })
}
