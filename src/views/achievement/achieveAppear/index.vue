  ...
  <div class="achieveAppear app-container">
    ...
    <el-table ref="multipleTable" :data="tableData" highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button type="primary" size="small" v-if="scope.row.approvalStatus == '0' || scope.row.approvalStatus == '3'" @click="achieveSubmit(scope.row)">上报</el-button>
          <el-button type="warning" size="small" v-if="scope.row.approvalStatus == '0' || scope.row.approvalStatus == '3'" @click="openUpdate(scope.row)">修改</el-button>
          <el-button type="danger" size="small" v-if="scope.row.approvalStatus == '0' || scope.row.approvalStatus == '3'" @click="achieveRemove(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getList, modify, removeItem } from '@/api/appr/achieveAppear'
...
export default {
  ...
  methods: {
    ...
    querySearch () {
      this.listQuery.queryStartTime = this.timeScope && this.timeScope[0]
        ? this.timeScope[0]
        : null
      this.listQuery.queryEndTime = this.timeScope && this.timeScope[1]
        ? this.timeScope[1]
        : null
      let _self = this
    ...
    // 点击上报按钮
    achieveSubmit (item) {
      modify({ id: item.id, approvalStatus: '1' }).then(
        ({ data: res }) => {
          notifySuccess(this, '上报成功')
          this.querySearch()
        }
      )
    },
    ...
    achieveRemove (item) {
      let _self = this
      removeItem(item.id).then(({ data: res }) => {
        if (res.data) {
          notifySuccess(_self, '删除成功')
          _self.querySearch()
        }
      })
    }
