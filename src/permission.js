...
router.beforeEach((to, from, next) => {
  ...
  if (store.getters.access_token) { // determine if there has token
    ...
    if (whiteList.indexOf(to.path) !== -1) { // 在免登录白名单，直接进入
      ...
      // hainanTokenPair = ["token", "tokenValue"]
      let hainanTokenPair = location.href.substr(location.href.indexOf('?token=') + 1).substr(0, location.href.substr(location.href.indexOf('?token=') + 1).indexOf('#')).split('=')
      if (hainanTokenPair.length > 1) {
        next({ path: '/login', query: { token: hainanTokenPair[1] } }) // 否则全部重定向到登录页
      } else {
