import { loginByUsername, loginByHinanToken, logout, getUserInfo } from '@/api/admin/login'
  ...
  actions: {
    ...
    LoginByUsername ({
      ...
      return new Promise((resolve, reject) => {
        ...
        // TODO 海南登录 loginByHinanToken 密码登录:loginByUsername
        // loginByHinanToken(user.tenantCode, user.identifier, user.credential, user.code, user.randomStr).then(response => {
    ...
    GetMenu ({ commit }) {
      return new Promise(resolve => {
        GetMenu().then((res) => {
          ...
          //   if (item.name == "评估分析" || item.name == "任务管理" || item.name == "评分管理" || item.name == "任务考核" || item.name == "会议管理") {
          //     data.push(item)
          //   }
          // })
          const data = res.data
